...
export const isSuccessRenderFistRendering = {
  action() {
    ...
    cy
      .get(`[data-cy=${CY_SELECTOR.accountItem}]`)
      .its('length')
      .should('be.gt', 0);
  },
  actionTest() {
...
export const isMoveAddPage = {
  action() {
    ...
    cy.location().should((location) => {
      expect(location.pathname).to.eq('/accounts/save');
    })
  },
  actionTest() {
  ...
  action() {
    ...
    cy
      ...
      .then(bucket => {
        ...
        cy.location().should((location) => {
          expect(location.pathname).to.eq('/accounts/29');
        })
      });
  },
  actionTest() {
    it('자산관리 아이템 클릭시 정상 상세 페이지 이동', () => this.action());
  }
};
